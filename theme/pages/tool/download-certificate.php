...
if (
  $response['id__'.$registration_form_id.'__'.$name_ques_id] &&
  (!$incomplete || ($chatbot['allow_incomplete_certificate_download'] ?? false))
) {
  $form_data = [
  ...
  echo "<strong style='color: red;'><em>Error</em></strong>: Certificate not available, share your id <strong>{$_GET['response_id']}</strong> with person in-charge for help";
}
